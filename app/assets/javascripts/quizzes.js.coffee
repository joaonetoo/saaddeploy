...
jQuery ->
    ...
    $('#_subject_id').on "change", ->
      ...
      $('#_classroom_id').on "change", ->
        ...
        if options
            $('#_user_id').html(options)
            $('#_user_id').prepend("<option value='todos'>Todos</option>")
            $('#_user_id option:first').attr("selected", "selected");
            $('#_user_id').parent().show()
        else
            $('#_user_id').empty()